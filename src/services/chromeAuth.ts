...
/**
 * 使用 Chrome Identity launchWebAuthFlow 进行 Slack OAuth 登录
 */
export const signInWithSlackIdentity = async (): Promise<void> => {
  try {
    console.log('🚀 [步骤 1/5] 开始 Slack OAuth 登录流程...');
    console.log('📍 Chrome Extension ID:', chrome.runtime.id);
    ...
    if (!access_token) {
      console.error('❌ 未找到 access_token');
      console.log('URL 参数:', Array.from(hashParams.entries()));
      throw new Error('未能从重定向 URL 获取 access token。请检查 Supabase 和 Slack OAuth 配置。');
    }
    ...
    console.log('🎉 Slack 登录完成！');
  } catch (error) {
    console.error('❌ Slack Identity 登录失败:', error);
    if (error instanceof Error) {
      console.error('错误详情:', error.message);
      console.error('错误堆栈:', error.stack);
    }
