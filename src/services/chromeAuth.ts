...
/**
 * 使用 Chrome Identity launchWebAuthFlow 进行 GitHub OAuth 登录
 */
export const signInWithGitHubIdentity = async (): Promise<void> => {
  try {
    console.log('🚀 [步骤 1/5] 开始 GitHub OAuth 登录流程...');
    console.log('📍 Chrome Extension ID:', chrome.runtime.id);
    ...
    if (!access_token) {
      console.error('❌ 未找到 access_token');
      console.log('URL 参数:', Array.from(hashParams.entries()));
      throw new Error('未能从重定向 URL 获取 access token。请检查 Supabase 和 GitHub OAuth 配置。');
    }
    ...
    console.log('🎉 GitHub 登录完成！');
  } catch (error) {
    console.error('❌ GitHub Identity 登录失败:', error);
    if (error instanceof Error) {
      console.error('错误详情:', error.message);
      console.error('错误堆栈:', error.stack);
    }
...
/**
 * 使用 Chrome Identity launchWebAuthFlow 进行 Twitter/X OAuth 登录
 */
export const signInWithTwitterIdentity = async (): Promise<void> => {
  try {
    console.log('🚀 [步骤 1/5] 开始 Twitter/X OAuth 登录流程...');
    console.log('📍 Chrome Extension ID:', chrome.runtime.id);
    ...
    if (!access_token) {
      console.error('❌ 未找到 access_token');
      console.log('URL 参数:', Array.from(hashParams.entries()));
      throw new Error('未能从重定向 URL 获取 access token。请检查 Supabase 和 Twitter OAuth 配置。');
    }
    ...
    console.log('🎉 Twitter/X 登录完成！');
  } catch (error) {
    console.error('❌ Twitter/X Identity 登录失败:', error);
    if (error instanceof Error) {
      console.error('错误详情:', error.message);
      console.error('错误堆栈:', error.stack);
    }
